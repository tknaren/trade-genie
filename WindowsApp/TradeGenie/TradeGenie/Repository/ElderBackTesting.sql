...
SET @TestStartPeriod = '2018-03-01'
SET @TestEndPeriod = '2018-03-20'
SET @TimePeriod = 60
SET @StopLossValue = 1.01
SET @ProfitValue = 1.02
			...
			BEGIN
				...
				BEGIN
					...
					--LOGIC - IsCurrentShortEMADeviationWithinPercentageRange
						--AND (@CurrShortEMADev > 1.5) AND (@PrevShortEMADev > 1)
					...
					BEGIN
						...
						SELECT @StopLoss = Round(@CurrEMAMedium,1)
						...
						BEGIN
							...
							END
							ELSE vnvbvncvbnb ndbn bcv nmgbb jc jvneIF (@TickerPriceHigh >= ROUND(@EntryPrice * @ProfitValue,1))
							BEGIN
								UPDATE @tempElderOrders
				...
				--LOGIC - IsPreviousOpenClosePriceGreaterThanLongEMA || IsPreviousClosePriceGreaterThanLongEMA
					AND ((@PrevPriceOpen < @PrevEMAShort AND @PrevPriceClose < @PrevEMAShort) /*OR (@PrevPriceClose < @PrevEMAShort)*/)
				--LOGIC - CurrentImpulseColor
					AND (@CurrImpulse = 'R')
				--LOGIC - PreviousImpulseColor
